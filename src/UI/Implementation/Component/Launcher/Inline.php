...
class Inline implements C\Launcher\Inline
{
    ...
    public function withInputs(Group $fields, \Closure $evaluation, MessageBox\MessageBox $instruction = null): self
    {
        ...
        $clone->form = $this->form_factory->standard(
            (string)$this->getTarget()->getURL() . '& ' . self::TOKENPARAM . '=' . $clone->getToken(),
            [$fields]
        );
    ...
    {
        $clone = clone $this;
        if (array_key_exists(self::TOKENPARAM, $request->getQueryParams())
            && $request->getQueryParams()[self::TOKENPARAM] === $this->getToken()
        ) {
