...
use ILIAS\UI\Implementation\Component\Legacy\Legacy as LegacyContent;
...
class ConsecutiveScoringGUI implements SegmentRetrieval
{
    ...
    public function getSegment(
        ...
        if ($position_data === null) {
            return $this->ui_factory->legacyFactory()->segment(
                '',
    ...
    protected function collectForUser(int $usr_active_id, array $question_ids): array
    {
        ...
        foreach ($question_ids as $question_id) {
            ...
            $entries[] = $this->ui_factory->legacy(sprintf('<a id="anchor_%s_%s"></a>', $question_id, $usr_active_id));
            $entries[] = $panel;
        }
        return $entries;
    }
